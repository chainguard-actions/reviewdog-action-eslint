...
curl -sfL -o "${TEMP_PATH}/install-reviewdog.sh" https://raw.githubusercontent.com/reviewdog/reviewdog/master/install.sh
sh "${TEMP_PATH}/install-reviewdog.sh" -b "${TEMP_PATH}" "${REVIEWDOG_VERSION}" 2>&1
echo '::endgroup::'
